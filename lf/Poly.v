...
Proof.
  intros X l.
  induction l as [| h1 t1 Hl1].
  - reflexivity.
  - simpl. rewrite -> rev_app_distr. rewrite -> Hl1. reflexivity.
Qed.
(** [] *)
...
Definition partition {X : Type}
                     (test : X -> bool)
                     (l : list X)
                   : list X * list X := (filter test l, filter (fun m => negb (test m)) l).
...
Proof.
  ...
  induction l as [| h t Hl].
  - reflexivity.
  - simpl. rewrite -> map_app_distr. rewrite -> Hl.
    simpl. reflexivity.
Qed.
(** [] *)
...
Definition exp (n m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) => m (X -> X) (fun i => n X i) (one X f) x.
