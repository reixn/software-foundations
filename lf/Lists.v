...
Proof.
  intros p. rewrite <- snd_fst_is_swap.
  reflexivity.
Qed.
(** [] *)
...
Theorem add_inc_count : 
  forall (n : nat) (b : bag), 1 + count n b = count n (add n b).
Proof.
  intros n b.
...
Proof.
  ...
  - reflexivity.
  - simpl. intros l2. rewrite -> Hl1.
    destruct h1 as [| h1'] eqn:Eh1.
    + reflexivity.
    + reflexivity.
Qed.
(** [] *)
...
Proof.
  induction s as [| h t Hs].
  - reflexivity.
  - destruct h as [| h'] eqn:Eh.
...
Theorem bag_count_sum: 
  forall (n : nat) (b1 b2 : bag), count n b1 + count n b2 = count n (sum b1 b2).
Proof.
  intros n b1 b2.
  induction b1 as [| h1 t1 Hb1].
  - reflexivity.
  - simpl. destruct (h1 =? n) eqn:Hh1.
...
Proof.
  intros f Hf n1 n2 H.
  rewrite -> Hf. replace (n1) with (f (f n1)).
...
Proof.
  intros l1 l2 H.
  rewrite <- rev_involutive. replace (l1) with (rev (rev l1)).
  - rewrite -> H. reflexivity.
...
Proof. intros x. destruct x. simpl. rewrite -> eqb_refl. reflexivity. Qed.
(** [] *)
